...
plt.plot(mgmnt_new, label="Mgmnt  T.Stu:"+str(len(mgmnt_new))+"  Max:"+str(np.amax(mgmnt_new))+"  Avg:"+str(round(np.average(mgmnt_new),2))+"  %Stu:"+str(round((len(mgmnt_new)/mgmnt_total*100),2)))
plt.plot(or_new, label='OR  T.Stu:'+str(len(or_new))+'  Max:'+str(np.amax(or_new))+'  Avg:'+str(round(np.average(or_new),2))+'  %Stu:'+str(round((len(or_new)/or_total*100),2)))
plt.plot(usp_new, label='USP  T.Stu:'+str(len(usp_new))+'  Max:'+str(np.amax(usp_new))+'  Avg:'+str(round(np.average(usp_new),2))+'  %Stu:'+str(round((len(usp_new)/usp_total*100),2)))
plt.plot(cd_new, label='CD  T.Stu:'+str(len(cd_new))+'  Max:'+str(np.amax(cd_new))+'  Avg:'+str(round(np.average(cd_new),2))+'  %Stu:'+str(round((len(cd_new)/cd_total*100),2)))
plt.plot(cn_new, label='CN  T.Stu:'+str(len(cn_new))+'  Max:'+str(np.amax(cn_new))+'  Avg:'+str(round(np.average(cn_new),2))+'  %Stu:'+str(round((len(cn_new)/cn_total*100),2)))
plt.plot(cg_new, label='CG  T.Stu:'+str(len(cg_new))+'  Max:'+str(np.amax(cg_new))+'  Avg:'+str(round(np.average(cg_new),2))+'  %Stu:'+str(round((len(cg_new)/cg_total*100),2)))

plt.legend()
plt.title("Reval data of 130 CSE 6th sem VTU students")
plt.show()
